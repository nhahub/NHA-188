...
def upload_to_gcs(local_path, gcs_path, max_retries=3):
    """Upload local directory to GCS with retry logic and timeout handling"""
    client = storage.Client.from_service_account_json(CREDENTIALS_PATH)
    bucket = client.bucket(GCS_BUCKET)
    ...
    for root, dirs, files in os.walk(local_path):
        for file in files:
            ...
            for attempt in range(max_retries):
                try:
                    print(f"Uploading {local_file_path} (attempt {attempt + 1}/{max_retries})")
                    
                    # Increase timeout and use chunked upload for large files
                    blob.upload_from_filename(
                        local_file_path,
                        timeout=300,  # 5 minutes timeout
                        if_generation_match=0  # Avoid conflicts
                    )
                    print(f"✅ Successfully uploaded: {gcs_file_path}")
                    break  # Success, break out of retry loop
                    
                except (ConnectionError, TimeoutError, Exception) as e:
                    if attempt == max_retries - 1:  # Last attempt
                        print(f"❌ Failed to upload {local_file_path} after {max_retries} attempts: {e}")
                        raise
                    else:
                        wait_time = (2 ** attempt)  # Exponential backoff: 1, 2, 4 seconds
                        print(f"⚠️ Upload attempt {attempt + 1} failed. Retrying in {wait_time} seconds...")
